let
    # Import the overall nixpkgs listing and our hello_mpi derivation
    pkgs = import <nixpkgs> {};
    hello_mpi = pkgs.callPackage ./hello_mpi {};
    ...
    mpirunArguments = let
        process = "${hello_mpi}/bin/hello_mpi";  # The process that will be distributed
        processesPerHost = 2;  # The number of processes to start on each node
        hostNames = [  # The names of the hosts to start the processes on
            "master"
            "head"
        ];

        totalProcesses = (builtins.length hostNames) * processesPerHost;
        hosts = builtins.concatStringsSep "," (map (host: host + ":" + toString processesPerHost) hostNames);  # Processes the hostnames into a --hosts argument
    ...
    in
         "--allow-run-as-root ${sshOption} ${oversubscribe} --host ${hosts} -np ${toString totalProcesses} ${process}";
...
in pkgs.nixosTest ({
    name = "hello_mpi_vms";  # The name of the test, doesn't really matter.
    ...
    testScript = ''
        ...
        with subtest("MPI Run"):
            ...
            results = master.succeed("${pkgs.mpi}/bin/mpirun ${mpirunArguments}")
